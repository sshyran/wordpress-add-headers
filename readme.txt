...
which your WordPress powered web site sends back to the clients when dynamic pages are requested.
This gives you control over the way your dynamic content is cached by web browsers or caching proxy servers. If configured properly for your specific needs, it can result in more efficient caching of your content and thus reduce the bandwidth consumption and server load.
 ...
 * archives (category, tag, custom taxonomy, author or date based archives, including paginated pages, are supported).
...
In case of content pages (posts, pages, custom post types), the time the content
has been last modified is determined by the time the content was last edited or
by the creation time of the latest comment, whichever of the two is newer.
...
By default, on search results and any page, that has been requested by an
authenticated user, the max age of the response is set to 0, so as to prevent it
from being cached.

By default, the max age and thus the expiration date of each page, which is
supposed to be cached, is set to one day (86400 seconds) after the time the
client accessed the resource, but this is configurable (see *configuration* section).
...
Be advised that if your web site includes contact forms, shopping carts or other special pages which contain elements that need to be refreshed on every visit, those web pages are not going to work correctly with the default configuration of the plugin. In such a case, the customization of the plugin configuration is mandatory for those special pages.
...
Yes. Despite all the mandatory requirements in knowledge, this plugin has been designed to just work. The default options have sensible default values that should work for the vast majority of web sites. However, if your web site includes pages 
